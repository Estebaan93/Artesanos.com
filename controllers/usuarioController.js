...
import {buscarAlbumesPorTituloOEtiqueta, buscarImagenesPorTituloOEtiqueta} from "../models/albumModel.js";
import {insertarUsuario, obtenerUsuarios, obtenerUsuarioPorEmail, obtenerUsuariosExcepto, buscarUsuariosDisponibles} from "../models/usuarioModel.js";
...
export const listarUsuariosParaSolicitar = async (req, res) => {
  try {
    ...
    res.render('logueado/lista', { usuarios }); 
  } catch (error) {
...
export const apiBuscarContenido = async (req, res) => {
  try {
    const termino = req.query.q || '';

    if (!termino.trim()) {
      return res.json({ albumes: [], imagenes: [] });
    }

    const [albumes, imagenes] = await Promise.all([
      buscarAlbumesPorTituloOEtiqueta(termino),
      buscarImagenesPorTituloOEtiqueta(termino),
    ]);

    res.json({ albumes, imagenes });
  } catch (error) {
    console.error("Error en apiBuscarContenido:", error);
